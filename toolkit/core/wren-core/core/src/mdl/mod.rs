...
mod test {
    ...
    async fn test_remote_function() -> Result<()> {
        ...
        .await?;
        assert_snapshot!(actual, @r#"SELECT add_two("Customer"."Custkey") FROM (SELECT "Customer"."Custkey" FROM (SELECT __source."Custkey" AS "Custkey" FROM datafusion."public".customer AS __source) AS "Customer") AS "Customer""#);
    ...
    async fn test_query_hidden_column() -> Result<()> {
        ...
        .await?;
        assert_snapshot!(actual,
                   @r#"SELECT artist."串接名字" FROM (SELECT artist."串接名字" FROM (SELECT __source."名字" || __source."名字" AS "串接名字" FROM artist AS __source) AS artist) AS artist"#);
        let sql = r#"select * from wren.test.artist"#;
        let actual = transform_sql_with_ctx(
            &create_wren_ctx(None, None),
            Arc::clone(&analyzed_mdl),
    ...
    async fn test_infer_timestamp_column() -> Result<()> {
        ...
        .await?;
        assert_snapshot!(actual,
                   @r#"SELECT CAST(current_date() AS TIMESTAMP) > artist."出道時間" FROM (SELECT artist."出道時間" FROM (SELECT __source."出道時間" AS "出道時間" FROM artist AS __source) AS artist) AS artist"#);
        Ok(())
    }
    ...
    async fn test_disable_count_wildcard_rule() -> Result<()> {
        ...
        assert_snapshot!(actual, @r#"SELECT count(1) AS "count(*)" FROM (SELECT 1)"#);
        Ok(())
    }
    ...
    async fn test_eval_timestamp_with_session_timezone() -> Result<()> {
        ...
        assert_snapshot!(actual, @r#"SELECT CAST('2011-01-01 18:00:00' AS TIMESTAMP) AS "Utf8(""2011-01-01 18:00:00"")""#);
        ...
        assert_snapshot!(actual, @r#"SELECT CAST('2011-01-01 10:00:00' AS TIMESTAMP) AS "Utf8(""2011-01-01 18:00:00"")""#);
        ...
        .await?;
        assert_snapshot!(actual, @r#"SELECT CAST('2024-01-15 23:00:00' AS TIMESTAMP) AS "Utf8(""2024-01-15 18:00:00"")""#);
        ...
        .await?;
        assert_snapshot!(actual, @r#"SELECT CAST('2024-07-15 22:00:00' AS TIMESTAMP) AS "Utf8(""2024-07-15 18:00:00"")""#);
    ...
    async fn test_disable_pushdown_filter() -> Result<()> {
        ...
        assert_snapshot!(actual,
                   // TODO: BigQuery doesn't support the alias include invalid characters (e.g. `*`, `()`).
                   //      We should remove the invalid characters for the alias.
                   @r#"SELECT count(1) AS "count(*)" FROM (SELECT artist.cast_timestamptz FROM (SELECT CAST(__source."出道時間" AS TIMESTAMP WITH TIME ZONE) AS cast_timestamptz FROM artist AS __source) AS artist) AS artist WHERE CAST(artist.cast_timestamptz AS TIMESTAMP) > CAST('2011-01-01 21:00:00' AS TIMESTAMP)"#);
        Ok(())
    }
    ...
    async fn test_list() -> Result<()> {
        ...
        .await?;
        assert_snapshot!(actual, @"SELECT list_table.list_col[1] FROM (SELECT list_table.list_col FROM (SELECT __source.list_col AS list_col FROM list_table AS __source) AS list_table) AS list_table");
        Ok(())
    }
    ...
    SELECT 2 x, 'c' y)"#;
        ...
        assert_snapshot!(
            result,
            @"SELECT x, y FROM (SELECT 1 AS x, 'a' AS y UNION ALL SELECT 1 AS x, 'b' AS y UNION ALL SELECT 2 AS x, 'a' AS y UNION ALL SELECT 2 AS x, 'c' AS y)"
        );
        Ok(())
    }
    ...
    async fn test_disable_single_distinct_to_group_by() -> Result<()> {
        ...
        assert_snapshot!(
            result,
            @"SELECT customer.c_custkey, count(DISTINCT customer.c_name) FROM (SELECT customer.c_custkey, customer.c_name FROM (SELECT __source.c_custkey AS c_custkey, __source.c_name AS c_name FROM customer AS __source) AS customer) AS customer GROUP BY customer.c_custkey"
        );
        Ok(())
    }
